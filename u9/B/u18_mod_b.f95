...
CONTAINS
  ...
  FUNCTION build_cycle (fname)
    implicit none
    character(*) :: fname
    TYPE(CYCLE) :: tempp, build_cycle
    INTEGER :: talter,ios
    CHARACTER(10) :: tname
    ...
    CALL init_cycle(build_cycle)
    ALLOCATE (Build_cycle%top)
    TemPp%top => Build_cycle%top
    ...
    Tempp%top%Next => build_cycle%top ! Letzter Eintrag Zeigt auf den Kopf
